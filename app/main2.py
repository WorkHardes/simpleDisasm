...
class DisasmArchiveStrategy(Strategy):

    def disasm_file(self, file_path: str, result_folder_name: str, file_type: str = None, file_content=None) -> None:
        result_of_extract_path = self.extract_archive(file_path)
        java_classes = []
...
# (file_path, result_folder_name, file_type, file_content)


class DisasmBinStrategy(Strategy):
...
if __name__ == "__main__":
    ...
    if "Zip archive data" in file_type:
        context = Context(DisasmArchiveStrategy())

    else:
        context = Context(DisasmBinStrategy())
