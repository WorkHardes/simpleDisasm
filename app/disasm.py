...
from services import open_file, extract_archive, disasm_with_dex2jar, disasm_and_save_result
    ...
    def disasm_file(self, file_path: str, result_folder_name: str = None) -> None:
        ...
        for root, dirs, files in os.walk(result_of_extraction_folder_path):
            for file_path in files:
                ...
                file_type = magic.from_buffer(file_content)
                if "ELF" in file_type:
                    executable_file_paths_list.append(pathlib.Path(file_path))
        ...
        if len(executable_file_paths_list) != 0:
            ...
            pathlib.Path(f"{PATH_OF_RESULTS_FOLDER}{result_folder_name}{asm_result_folder_name}").mkdir(
